...
request to `{base_url}/responses` and reads the provider's model catalog from
`{base_url}/models`; the key stays in the user's SQLite database and is never
returned to the browser. Existing OpenAI-LB credentials remain supported for
users who have not migrated. Optional completion and failure notifications use a
user-owned Linkit Bot and that same subject's private conversation. Notification
setup and delivery are independent of model inference.
