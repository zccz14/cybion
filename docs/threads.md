...
Terminal handoff is durably claimed before compaction starts. At most one terminal-handoff job exists for a child thread, and the child checkpoint plus paired main-thread join outcome are committed in one SQLite transaction. A process restart releases an unfinished claim and resumes the same durable job; it never treats a compaction output or child checkpoint as a new terminal handoff source.
