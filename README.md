...
当前实现始终保存完整的主线程消息和工具执行记录。上下文达到编译阈值时，Mobius 会用
模型生成 checkpoint，并将它与其后的历史切片组成稳定前缀；checkpoint 请求失败时继续
使用完整历史，不会把压缩结果提升为第二份事实来源。控制台会显示当前 checkpoint 和
完整历史消息数。服务重启时只恢复尚未开始执行的输入；已经开始调用工具的运行会明确
标记失败而不会自动重放，以免重复产生副作用。
...
- 自动上下文 checkpoint、稳定前缀编译和完整历史追溯；工具参数与结果也是历史来源。
...
- 对 checkpoint 压缩质量、长期事实召回和稳定前缀缓存命中率建立回放评测，并按模型
  上下文窗口动态调整编译预算。
