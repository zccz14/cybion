...
![从桌面发起目标、移动中补充指令，到回到同一主线程查看证据的 Cybion 使用漫画](docs/images/usability-comic.png)

一个典型使用路径是：

1. 在桌面打开同一条主线程，直接说明想达成的结果；无需先创建项目或 Session。
2. Agent 根据工具反馈读取文件、运行命令或 fork 独立的 Goal；操作者可在同一处查看进展与证据。
3. 离开桌面后，用手机、平板或连续语音输入追加约束、补充信息，或对需要判断的事项作出决定。
4. 回到任意已登录浏览器时，继续查看同一条对话的结果；工具操作仍发生在已授权的本机或远程设备上。
