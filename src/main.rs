...
const THREAD_PAGE_DEFAULT: usize = 20;
const THREAD_PAGE_MAX: usize = 100;
...
const SUBTHREAD_NO_PROGRESS_MAX_ATTEMPTS: i64 = 2;
...
async fn run_subthread(state: AppState, job: QueuedSubthread) {
    ...
    match result {
        Ok(result) => {
            if !result.useful_progress {
                match record_subthread_no_progress(&state.db_path, &job.id) {
                    Ok(attempt) if attempt >= SUBTHREAD_NO_PROGRESS_MAX_ATTEMPTS => {
                        let _ = block_goal(
                            &state.db_path,
                            &job.id,
                            "The Goal was blocked after repeated acknowledgement-only responses.",
                            "The model repeatedly acknowledged developer/runtime context without taking a Goal step, calling a tool, or reaching a terminal state. The durable Goal and history were retained; retry after correcting the upstream prompt/model behavior.",
                        );
                    ...
                    Ok(_) => {}
                    Err(cause) => {
                        tracing::warn!(%cause, subthread = %job.id, "cannot record subthread no-progress response")
                    }
                }
            } else {
                let _ = reset_subthread_no_progress_after_useful_progress(&state.db_path, &job.id);
            }
...
fn create_or_resume_compaction_job(
    ...
    let existing = if terminal_handoff {
        connection
            .query_row(
                "SELECT id FROM compaction_jobs
                 WHERE thread_id IS ?1 AND purpose='terminal_handoff'
                 ORDER BY id DESC LIMIT 1",
                [&audit.thread_id],
                |row| row.get(0),
            )
...
fn compile_subthread_context(
    ...
    let mut records = load_protocol_items(&connection, None, idx_head, fork_from_id)?
        .into_iter()
        .map(|(id, kind, created_at, item)| {
            (id, kind, created_at, subthread_inherited_context_item(item))
        })
        .collect::<Vec<_>>();
...
fn subthread_inherited_context_item(item: Value) -> Value {
    ...
    json!({
        "role": "developer",
        "content": format!(
            "## Inherited main-thread context\n\n{content}\n\nThis is background context, not a task for this Goal."
        ),
    })
}
...
fn terminal_subthread_handoff_output(
    ...
    Ok(serde_json::to_string(&json!({
        ...
        "handoff_checkpoint": handoff.zip(checkpoint_id).map(|(value, checkpoint_id)| json!({
            "checkpoint_id": checkpoint_id,
            "markdown": value.markdown,
            "source_range": [value.idx_head, value.idx_tail],
            "compaction_output_record_id": value.output_record_id,
        })),
        "handoff_checkpoint_status": if checkpoint_id.is_some() { "available" } else { "unavailable" },
        "retrieval": {
            "thread_id": join.id,
            "fork_record_id": join.from_record_id,
            "history_range": [join.from_record_id, handoff.zip(checkpoint_id).map(|(value, _)| value.idx_tail).unwrap_or(join.from_record_id)],
...
fn finalize_terminal_subthread_join(
        ...
        .query_row(
            "SELECT id, from_record_id, goal_state, goal_evidence, blocked_reason, COALESCE(result, '')
             ...
             WHERE id = ?1
               ...
               AND terminal_join_started_at IS NOT NULL",
            [id],
            |row| {
    ...
    let checkpoint_id = match handoff {
        Some(handoff) if latest_protocol_record_id(&transaction, Some(id))? == handoff.idx_tail => {
            let summary =
                checkpoint_text_from_output_record(&transaction, handoff.output_record_id)?;
            Some(history_record_payload(
                &transaction,
                Some(id),
                "checkpoint",
                &compacted_checkpoint_item(&summary),
                &chrono::Utc::now().to_rfc3339(),
            )?)
        }
        _ => None,
    ...
    let now = chrono::Utc::now().to_rfc3339();
    let output = terminal_subthread_handoff_output(&pending, handoff, checkpoint_id)?;
    ...
    let changed = transaction.execute(
        "UPDATE subthreads
         SET status = 'completed', outcome_record_id = ?1, handoff_checkpoint_id = ?2,
             terminal_join_started_at = NULL, next_retry_at = NULL, retry_attempt = 0, updated_at = ?3
         WHERE id = ?4
           AND goal_state IN ('achieved', 'blocked', 'cancelled')
           AND status != 'completed'
           AND outcome_record_id IS NULL
           AND terminal_join_started_at IS NOT NULL",
        params![outcome_record_id, checkpoint_id, now, id],
    )?;
    if changed != 1 {
...
fn claim_terminal_subthread_join(path: &Path, id: &str) -> Result<bool> {
    let changed = open_db(path)?.execute(
        "UPDATE subthreads SET terminal_join_started_at = ?1, updated_at = ?1
         WHERE id = ?2
           AND goal_state IN ('achieved', 'blocked', 'cancelled')
           AND status != 'completed'
           AND outcome_record_id IS NULL
           AND terminal_join_started_at IS NULL",
...
async fn start_terminal_subthread_join(state: &AppState, id: &str) -> Result<bool> {
    if !claim_terminal_subthread_join(&state.db_path, id)? {
        return Ok(false);
    }
...
fn is_subthread_acknowledgement_only(text: &str) -> bool {
    let normalized = text
        .trim()
    ...
    matches!(
        normalized.as_str(),
        "已确认" | "确认" | "confirmed" | "acknowledged" | "i confirm"
    ) || normalized.starts_with("已确认当前日期为 ")
        || normalized.starts_with("the current date is ")
}

fn record_subthread_no_progress(path: &Path, id: &str) -> Result<i64> {
    let mut connection = open_db(path)?;
    let transaction = connection.transaction()?;
    let attempt = transaction.query_row(
        "SELECT no_progress_attempt + 1 FROM subthreads
         WHERE id = ?1 AND goal_state = 'active' AND status = 'running'",
        [id],
        |row| row.get::<_, i64>(0),
    )?;
    transaction.execute(
        "UPDATE subthreads SET no_progress_attempt = ?1, updated_at = ?2 WHERE id = ?3",
        params![attempt, chrono::Utc::now().to_rfc3339(), id],
    )?;
    transaction.commit()?;
    Ok(attempt)
}

fn reset_subthread_no_progress_after_useful_progress(path: &Path, id: &str) -> Result<()> {
    open_db(path)?.execute(
        "UPDATE subthreads SET no_progress_attempt = 0 WHERE id = ?1 AND status = 'running'",
        [id],
    )?;
    Ok(())
}
...
fn migrate_subthread_scheduler_schema(connection: &Connection) -> Result<()> {
    ...
    if !columns
        .iter()
        .any(|column| column == "terminal_join_started_at")
    {
        connection
            .execute_batch("ALTER TABLE subthreads ADD COLUMN terminal_join_started_at TEXT;")?;
    }
    // RECOVERY: a process cannot retain an in-flight terminal handoff across restart. The
    // completed compaction job is durable and will be reused when reconciliation claims it again.
    connection.execute(
        "UPDATE subthreads SET terminal_join_started_at = NULL
         WHERE terminal_join_started_at IS NOT NULL
           AND status != 'completed' AND outcome_record_id IS NULL",
        [],
    )?;
...
fn migrate_compaction_schema(connection: &Connection) -> Result<()> {
    ...
    connection.execute_batch(
        "UPDATE compaction_jobs AS stale
         SET purpose = 'terminal_handoff_superseded'
         WHERE stale.purpose = 'terminal_handoff'
           AND EXISTS (
             SELECT 1 FROM compaction_jobs AS newer
             WHERE newer.purpose = 'terminal_handoff'
               AND newer.thread_id IS stale.thread_id
...
async fn executor_tunnel_resource(
    ...
    if sessions
        .get(&machine_id)
        .is_none_or(|session| session.id != report.epoch)
    {
        return Err(error(
            StatusCode::CONFLICT,
...
fn load_subthreads(path: &Path) -> Result<Vec<Subthread>> {
    load_subthreads_matching(path, "1 = 1", &[])
}

fn load_subthreads_matching(
    path: &Path,
    predicate: &str,
    values: &[SqlValue],
) -> Result<Vec<Subthread>> {
    open_db(path)?
        .prepare(&format!(
...
fn thread_page_size(value: Option<usize>) -> Result<usize> {
    let page_size = value.unwrap_or(THREAD_PAGE_DEFAULT);
    if !(1..=THREAD_PAGE_MAX).contains(&page_size) {
        return Err(anyhow!("page_size must be between 1 and {THREAD_PAGE_MAX}"));
    }
    Ok(page_size)
}

fn thread_page(value: Option<usize>) -> Result<usize> {
...
) -> ApiResult<ThreadIndex> {
    identity(&state, &headers).await?;
    load_thread_index(&state.db_path, query)
        .map(Json)
        .map_err(|cause| error(StatusCode::BAD_REQUEST, cause.to_string()))
}
...
async fn run_agent_items(
    ...
    let mut made_tool_progress = false;
    ...
    loop {
        ...
        let response_input_tokens = response
            .pointer("/usage/input_tokens")
            .and_then(Value::as_u64);
        if let Some(response_input_tokens) = response_input_tokens {
            send_agent_event(
                db_path,
                &events,
                AgentEvent::Context {
        ...
        made_tool_progress |= output.iter().any(|item| {
            matches!(
        ...
        if !output.iter().any(|item| {
            ...
            return Ok(AgentResult {
                useful_progress: made_tool_progress
                    || !is_subthread_acknowledgement_only(&persisted_message.content),
                persisted_message,
                #[cfg(test)]
                message: ChatMessage {
                    role: "assistant".to_owned(),
...
fn scoped_responses_request_body(
    ...
    let tools = body
        ...
        .as_array_mut()
        .expect("tool definitions are an array");
    tools.extend(thread_tool_definitions());
    body["tool_choice"] = Value::String("auto".to_owned());

    let developer_sections = vec![skill_developer_section(skills), thread_developer_section()];
    let mut developer_sections = developer_sections;
...
fn thread_tool_definitions() -> Vec<Value> {
    ...
    tools.extend(
        [
            json!({"type":"function","name":"achieve_goal","description":"Mark your own persistent Goal achieved. Call this only when every done-when criterion is met. Write result and evidence in the user-preferred language inferred from the inherited conversation context. result is the concise terminal handoff; evidence must be concise and verifiable. This tool has no Goal ID because it always applies to your current Goal.","parameters":{"type":"object","additionalProperties":false,"required":["result","evidence"],"properties":{"result":{"type":"string"},"evidence":{"type":"string"}}}}),
            json!({"type":"function","name":"block_goal","description":"Mark your own persistent Goal blocked. Call this only when an external change or decision is required before progress can continue. Write result and reason in the user-preferred language inferred from the inherited conversation context. result is the concise terminal handoff. This tool has no Goal ID because it always applies to your current Goal.","parameters":{"type":"object","additionalProperties":false,"required":["result","reason"],"properties":{"result":{"type":"string"},"reason":{"type":"string"}}}}),
        ]);
    tools
}

fn thread_developer_section() -> String {
    let subthread_language_policy = "## Subthread language\n\nInfer the user-preferred language from the current conversation context, including the user’s current language and any explicit preference. For every fork, use it for title, task, and completion_criteria. Do not derive it from the UI locale or source-code language, and do not add a caller-provided language field. When a subthread calls achieve_goal or block_goal, write its user-visible result, evidence, or reason in that same inferred language so the terminal handoff remains consistent.";
    let subthread_parallelism_policy = "## Subthread parallelism\n\nFor independently executable work, subthread parallelism has unlimited default priority: actively fork all independent substantial tasks that can make useful progress in parallel. Do not serialize or cancel independent Goals because they share a repository, may touch the same files, may create merge conflicts, or may eventually share a release or deploy chain. Give each Goal an independent worktree and branch. Resolve ordinary integration through Git linear history, rebase, and normal CI. Cancel an existing Goal only when a later user instruction makes its objective, premise, or expected correct result irrelevant, wrong, unusable, or directly conflicting. Ordinary Git integration conflicts are not a reason to cancel a Goal. Parallelism does not bypass user instructions, security constraints, credentials policy, production safeguards, correct isolation, verification, or delivery sequencing.";
    format!(
        "{subthread_language_policy}\n\n## Thread role\n\nIf the current thread is the main thread, it carries the single user-visible durable conversation. Accept every user input as part of that conversation. For a complex request, normalize and preserve the user's complete destination as one Big Goal. Do not narrow that destination into a smaller phase, scaffold, audit, research result, or partial implementation merely to dispatch it. A complete Big Goal may be forked to one subthread; the main thread does not act as a long-term planner and does not create planner modeling such as requirement graphs, coverage databases, automatic phase splitting, or milestone orchestration. Use direct tools for brief, localized checks or edits. Every fork is one persistent Goal and must provide a durable objective plus concrete done-when criteria. When selecting an optional fork_subthread model_id, prefer gpt-5.6-sol for scientific or deep research work, gpt-5.6-terra for engineering work, and gpt-5.6-luna for operational or simple low-ambiguity work. This is guidance, not a substitute for judgment. Inspect existing Goals before replacing work; apply the Subthread parallelism cancellation rule before cancelling any Goal. Cybion returns only an achieved or blocked Goal handoff and resumes automatically. Never claim the user objective is complete merely because a Goal was dispatched, and never ask the user to manage Goals as sessions.\n\nIf the current thread is a subthread, it executes the inherited Goal from a compiled main-thread checkpoint. The persistent Goal is the only user task; inherited main-thread material and all developer/runtime metadata are background constraints, never a prompt to acknowledge, confirm, or repeat a date. One more step is the executing thread's local discipline: compare the current verified state with the complete Big Goal, take the next useful verifiable step toward that destination, and reassess from the new evidence. It never authorizes narrowing or pre-splitting the user's destination. Keep taking the next useful step until every done-when criterion for the complete Goal is met or further progress is blocked by a concrete external change. Do not report a safe skeleton, research result, or first milestone as completion of the complete Goal. A natural-language response is only progress and will start another loop. Call achieve_goal with a concise final result and verifiable evidence when the Goal is achieved, or block_goal with a concise final result and the concrete blocker when it cannot progress. After either terminal tool, take no further action. Do not ask the user to manage this branch.\n\n{subthread_parallelism_policy}\n\nUse `search_thread_history`, `read_thread_history`, and `get_checkpoint` when you need older information."
    )
}
...
async fn execute_tool(
    ...
    match name {
        ...
        "list_subthreads" => tool_execution("error: subthreads cannot list subthreads"),
        "fork_subthread" => tool_execution("error: subthreads cannot create subthreads"),
        "cancel_subthread" => tool_execution("error: subthreads cannot cancel subthreads"),
        "retry_subthread" => tool_execution("error: subthreads cannot retry subthreads"),
        "achieve_goal" => {
            tool_execution("error: main threads cannot mark a subthread Goal achieved")
        }
        "block_goal" => tool_execution("error: main threads cannot mark a subthread Goal blocked"),
...
mod tests {
    ...
    #[test]
    fn acknowledgement_only_subthread_text_recognizes_runtime_dates_without_matching_goal_progress()
    {
        assert!(is_subthread_acknowledgement_only("已确认。"));
        assert!(is_subthread_acknowledgement_only(
            "已确认当前日期为 2026 年 8 月 27 日，星期四。"
        ));
        assert!(is_subthread_acknowledgement_only(
            "The current date is August 27, 2026."
        ));
        assert!(!is_subthread_acknowledgement_only(
            "I ran the first check; the external lock is still held."
        ));
    }
    ...
    #[test]
    fn fork_subthread_uses_the_user_preferred_language_without_a_language_parameter() {
        let temp = tempfile::tempdir().unwrap();
        let db = temp.path().join("default.sqlite3");
        bootstrap_database(&db).unwrap();
        ...
        let main_developer = main["input"][0]["content"].as_str().unwrap();
        let subthread_developer = subthread["input"][0]["content"].as_str().unwrap();
        for prompt in [main_developer, subthread_developer] {
            assert!(prompt.contains(
                "Infer the user-preferred language from the current conversation context"
            ));
            assert!(prompt.contains("user’s current language and any explicit preference"));
            assert!(prompt.contains("Do not derive it from the UI locale or source-code language"));
            assert!(prompt.contains("do not add a caller-provided language field"));
        }
        assert!(subthread_developer.contains("user-visible result, evidence, or reason"));

        let main_tools = main["tools"].as_array().unwrap();
        let subthread_tools = subthread["tools"].as_array().unwrap();
        let fork = main_tools
            .iter()
            .find(|tool| tool["name"] == "fork_subthread")
            .unwrap();
        assert!(
            fork["description"]
                .as_str()
                .unwrap()
                .contains("use it for title, task, and completion_criteria")
        );
        let properties = fork["parameters"]["properties"].as_object().unwrap();
        assert!(!properties.contains_key("language"));
        for field in ["title", "task", "completion_criteria"] {
            assert!(properties[field]["description"].as_str().unwrap().contains(
                "user-preferred language inferred from the current conversation context"
            ));
        }
        for name in ["achieve_goal", "block_goal"] {
            for tools in [main_tools, subthread_tools] {
                assert!(
                    tools
                        .iter()
                        ...
                        .unwrap()
                        .contains(
                            "user-preferred language inferred from the inherited conversation context"
                        )
                );
            }
        }
    ...
    fn thread_index_keeps_main_thread_first_class_and_models_independent() {
        ...
        let empty = load_thread_index(&db, ThreadQuery::default()).unwrap();
        ...
        assert_eq!(
            load_thread_index(&db, ThreadQuery::default())
                .unwrap()
                .main_thread
                .status,
            "idle"
    ...
    fn thread_status_queries_filter_and_paginate_goals_server_side() {
        ...
        let user = append_conversation(
            ...
            &ChatMessage {
                role: "user".to_owned(),
                content: Value::String("create goals".to_owned()),
                images: None,
                tool_call_id: None,
                tool_calls: None,
            },
        ...
        assert_eq!(blocked.subthreads[0].goal_state, "blocked");
        assert!(
            load_thread_index(
                &db,
                ThreadQuery {
    ...
    #[test]
    fn main_and_subthread_requests_share_the_same_prefix_and_tools() {
        let temp = tempfile::tempdir().unwrap();
        let db = temp.path().join("default.sqlite3");
        bootstrap_database(&db).unwrap();
        let main = scoped_responses_request_body(
        ...
        assert_eq!(
            main["input"][0]["content"],
            subthread["input"][0]["content"]
        );
        assert_eq!(main["tools"], subthread["tools"]);
        assert_eq!(main["tool_choice"], "auto");
        let names = main["tools"]
            .as_array()
            .unwrap()
            .iter()
        ...
        assert!(developer.contains("If the current thread is the main thread"));
        assert!(developer.contains("If the current thread is a subthread"));
    }

    #[tokio::test]
    async fn thread_specific_tools_return_clear_scope_errors_without_side_effects() {
        let temp = tempfile::tempdir().unwrap();
        let db = temp.path().join("default.sqlite3");
        bootstrap_database(&db).unwrap();
        let active_subthreads = Arc::new(Mutex::new(HashMap::new()));
        let client = reqwest::Client::new();
        let skills = Arc::new(StdRwLock::new(SkillCatalog::default()));
        let tunnels = ExecutorTunnels::default();
        let call = |name, args, scope| {
            execute_tool(
                name,
                args,
                &db,
        ...
        assert_eq!(
            call("fork_subthread", json!({}), AgentScope::Subthread)
                .await
                .output,
            "error: subthreads cannot create subthreads"
        );
        assert_eq!(
            call(
                "cancel_subthread",
            ...
            .await
            .output,
            "error: subthreads cannot cancel subthreads"
        );
        assert_eq!(
            call("achieve_goal", json!({}), AgentScope::Main)
                .await
                .output,
            "error: main threads cannot mark a subthread Goal achieved"
        );
        assert_eq!(
            call("block_goal", json!({}), AgentScope::Main).await.output,
            "error: main threads cannot mark a subthread Goal blocked"
        );
        assert!(load_subthreads(&db).unwrap().is_empty());
    }

    #[test]
    fn main_and_subthread_prompts_share_the_unlimited_parallelism_policy() {
        let temp = tempfile::tempdir().unwrap();
        let db = temp.path().join("default.sqlite3");
        bootstrap_database(&db).unwrap();
        let main = scoped_responses_request_body(
        ...
        for phrase in [
            "unlimited default priority",
            "share a repository, may touch the same files, may create merge conflicts",
            "release or deploy chain",
            "independent worktree and branch",
            "Git linear history, rebase, and normal CI",
            "only when a later user instruction makes its objective, premise, or expected correct result irrelevant, wrong, unusable, or directly conflicting",
            "Ordinary Git integration conflicts are not a reason to cancel a Goal",
            "does not bypass user instructions, security constraints, credentials policy, production safeguards",
        ] {
            assert!(
        ...
        let fork_description = main["tools"]
            ...
            .unwrap();
        assert!(fork_description.contains("unlimited default priority"));
        assert!(
            fork_description.contains(
                "share a repository, may touch the same files, may create merge conflicts"
            )
        );
        assert!(fork_description.contains("independent worktree and branch"));
        assert!(fork_description.contains("Git linear history, rebase, and normal CI"));
        let cancel_description = main["tools"]
            .as_array()
            .unwrap()
            .iter()
            .find(|tool| tool["name"] == "cancel_subthread")
            .and_then(|tool| tool["description"].as_str())
            .unwrap();
        assert!(cancel_description.contains("later user instruction"));
        assert!(
            cancel_description
                .contains("Ordinary Git integration conflicts are not a reason to cancel a Goal")
        );
    }
    ...
    #[tokio::test]
    async fn acknowledgement_only_subthread_responses_block_after_a_bounded_replay() {
        async fn responses(
            State(requests): State<Arc<Mutex<Vec<Value>>>>,
            Json(request): Json<Value>,
        ) -> String {
            ...
            let item = json!({
                "type":"message",
                "role":"assistant",
                "content":[{"type":"output_text","text":"已确认当前日期为 2026 年 8 月 27 日，星期四。"}],
            });
            format!(
                "event: response.output_item.done\ndata: {}\n\nevent: response.completed\ndata: {}\n\n",
                json!({"type":"response.output_item.done","item":item}),
        ...
        configure_test_database(&db, &format!("http://{address}"));
        let connection = open_db(&db).unwrap();
        let metadata = history_record_payload(
            &connection,
            None,
            "input",
            &json!({"role":"developer","content":"The current date is Thursday, August 27, 2026."}),
            "now",
        )
        .unwrap();
        let user = append_conversation(
            ...
            &ChatMessage {
                role: "user".to_owned(),
                content: Value::String("Fix the background service.".to_owned()),
                images: None,
                tool_call_id: None,
                tool_calls: None,
            },
        ...
        .unwrap();
        assert!(metadata < user.id);
        let fork = execute_fork_subthread(
            &db,
            user.id,
            json!({"title":"Persistent fix","task":"Repair the service without acknowledging metadata.","completion_criteria":"The repair is verified."}),
        );
        let id = serde_json::from_str::<Value>(&fork.output).unwrap()["id"]
            .as_str()
            .unwrap()
            .to_owned();
        let first = claim_queued_subthreads(&db).unwrap().remove(0);
        run_subthread(test_state(db.clone()), first).await;
        let first_goal = load_subthread_detail(&db, &id).unwrap().unwrap().thread;
        assert_eq!(first_goal.goal_state, "active");
        assert_eq!(first_goal.status, "queued");
        assert_eq!(
            open_db(&db)
                .unwrap()
                .query_row(
                    "SELECT no_progress_attempt FROM subthreads WHERE id=?1",
                    [&id],
                    |row| row.get::<_, i64>(0)
                )
                .unwrap(),
        ...
        bootstrap_database(&db).unwrap();
        let second = claim_queued_subthreads(&db).unwrap().remove(0);
        run_subthread(test_state(db.clone()), second).await;
        server.abort();
        ...
        assert!(
            goal.result
                .as_deref()
                .is_some_and(|result| result.contains("acknowledgement-only"))
        );
        assert_eq!(requests.lock().await.len(), 3);
        let first_request = requests.lock().await[0].clone();
        let user_inputs = first_request["input"]
            .as_array()
            .unwrap()
            .iter()
        ...
        assert!(
            user_inputs[0]["content"]
                .as_str()
                .unwrap()
                .contains("Repair the service")
        );
        assert!(
            first_request["input"]
                .as_array()
                ...
                .any(|item| {
                    item["role"] == "developer"
                        && item["content"].as_str().is_some_and(|content| {
                            content.contains("The persistent Goal is the only user task")
                        })
                })
        );
        let own_records: i64 = open_db(&db)
            .unwrap()
            .query_row(
                "SELECT COUNT(*) FROM history_records WHERE thread_id=?1 AND kind='tool_output'",
                [&id],
                |row| row.get(0),
            )
            .unwrap();
        assert_eq!(own_records, 0);
    }
    ...
    async fn natural_language_goal_progress_requeues_the_same_subthread() {
        ...
        open_db(&db)
            .unwrap()
            .execute(
                "UPDATE subthreads SET no_progress_attempt = 1 WHERE id = ?1",
                [&id],
            )
            .unwrap();
        run_subthread(test_state(db.clone()), job).await;
    ...
    fn blocked_goal_result_remains_queryable() {
        ...
        .unwrap();
        assert!(claim_terminal_subthread_join(&db, &id).unwrap());
        assert!(
    ...
    fn terminal_handoff_checkpoint_stays_in_child_and_replays_to_main_as_a_paired_evidence_record()
    {
        ...
        let checkpoint_markdown = "# Durable working context\n\n## Concepts and terminology\n- Handoff.\n\n## Resources and authoritative locations\n- child.\n\n## Chronicle timeline\n- terminal.\n\n## Active decisions and constraints\n- preserve.\n\n## Current objective and next step\n- parent continues.\n\n## Open work and evidence routes\n```json\n[]\n```";
        let connection = open_db(&db).unwrap();
        let output_record_id = history_record_payload(
            &connection, Some(&id), "response_output",
            &json!({"type":"message","role":"assistant","content":[{"type":"output_text","text":checkpoint_markdown}]}),
            "2026-08-22T00:00:01Z",
        ).unwrap();
        connection.execute(
            "INSERT INTO compaction_jobs (thread_id,idx_head,idx_tail,status,purpose,final_output_record_id,created_at,updated_at) VALUES (?1,?2,?3,'completed','terminal_handoff',?4,'now','now')",
            params![id, fork_record_id, child_tail, output_record_id],
        ).unwrap();
        let job_id = connection.last_insert_rowid();
        connection.execute(
            "INSERT INTO compaction_nodes (job_id,output_record_id,source_records_json,created_at) VALUES (?1,?2,'[]','now')",
            params![job_id, output_record_id],
        ).unwrap();
        let handoff = TerminalSubthreadHandoff {
            markdown: checkpoint_markdown.to_owned(),
        ...
        let outcome = finalize_terminal_subthread_join(&db, &id, Some(&handoff))
            ...
            .unwrap();
        let connection = open_db(&db).unwrap();
        let checkpoint_id: i64 = connection
            .query_row(
                "SELECT handoff_checkpoint_id FROM subthreads WHERE id=?1",
                [&id],
                |row| row.get(0),
            )
            .unwrap();
    ...
    #[test]
    fn terminal_handoff_claim_allows_one_join_and_one_job_per_thread() {
        let temp = tempfile::tempdir().unwrap();
        let db = temp.path().join("default.sqlite3");
        bootstrap_database(&db).unwrap();
        let fork_record_id = history_record_payload(
            ...
            None,
            "response_output",
            &json!({"type":"function_call","call_id":"fork","name":"fork_subthread","arguments":"{}"}),
            "2026-08-22T00:00:00Z",
        )
        .unwrap();
        let fork = execute_fork_subthread(
            &db,
            fork_record_id,
            json!({"title":"One handoff","task":"Stop after one handoff","completion_criteria":"One join."}),
        );
        let id = serde_json::from_str::<Value>(&fork.output).unwrap()["id"]
            .as_str()
            .unwrap()
            .to_owned();
        let _ = claim_queued_subthreads(&db).unwrap();
        block_goal(&db, &id, "External decision required.", "Waiting.").unwrap();

        assert!(claim_terminal_subthread_join(&db, &id).unwrap());
        assert!(!claim_terminal_subthread_join(&db, &id).unwrap());
        assert!(pending_terminal_subthread_ids(&db).unwrap().is_empty());
        ...
        let job = create_or_resume_compaction_job(&db, &audit, true).unwrap();
        let later = ResponseAuditContext::for_request(
            "compaction",
            Some(id.clone()),
            Some(fork_record_id + 1),
            Some(fork_record_id + 1),
        );
        assert_eq!(
            create_or_resume_compaction_job(&db, &later, true).unwrap(),
            job
        );
        assert_eq!(
            open_db(&db)
                .unwrap()
                .query_row(
                    "SELECT COUNT(*) FROM compaction_jobs WHERE thread_id=?1 AND purpose='terminal_handoff'",
                    [&id],
                    |row| row.get::<_, i64>(0),
                )
                .unwrap(),
    ...
    #[test]
    fn terminal_handoff_claim_is_recovered_only_after_restart() {
        let temp = tempfile::tempdir().unwrap();
        let db = temp.path().join("default.sqlite3");
        bootstrap_database(&db).unwrap();
        let fork_record_id = history_record_payload(
            &open_db(&db).unwrap(), None, "response_output",
            &json!({"type":"function_call","call_id":"fork","name":"fork_subthread","arguments":"{}"}), "2026-08-22T00:00:00Z",
        ).unwrap();
        let fork = execute_fork_subthread(
            &db,
            fork_record_id,
            json!({"title":"Recover","task":"Recover once","completion_criteria":"One retry."}),
        );
        let id = serde_json::from_str::<Value>(&fork.output).unwrap()["id"]
            .as_str()
            .unwrap()
            .to_owned();
        let _ = claim_queued_subthreads(&db).unwrap();
        achieve_goal(&db, &id, "Done.", "Verified.").unwrap();
        assert!(claim_terminal_subthread_join(&db, &id).unwrap());
        assert!(pending_terminal_subthread_ids(&db).unwrap().is_empty());
        bootstrap_database(&db).unwrap();
        assert_eq!(pending_terminal_subthread_ids(&db).unwrap(), vec![id]);
    }
    ...
    fn developer_prompt_encodes_the_one_more_step_philosophy() {
        ...
        assert!(developer.contains("complete destination as one Big Goal"));
        assert!(developer.contains("does not act as a long-term planner"));
        assert!(developer.contains("It never authorizes narrowing or pre-splitting"));
        assert!(
            developer
                .contains("Do not report a safe skeleton, research result, or first milestone")
        );
        assert!(body.get("instructions").is_none());
    }

    #[test]
    fn fork_and_readme_preserve_big_goals_without_planner_modeling() {
        let temp = tempfile::tempdir().unwrap();
        let db = temp.path().join("default.sqlite3");
        bootstrap_database(&db).unwrap();
        let body = scoped_responses_request_body(
        ...
        let fork_description = body["tools"]
            .as_array()
            .unwrap()
            .iter()
            .find(|tool| tool["name"] == "fork_subthread")
            .and_then(|tool| tool["description"].as_str())
            .unwrap();
        assert!(fork_description.contains("complete normalized Big Goal"));
        assert!(fork_description.contains("whole user destination"));
        ...
        for phrase in [
            ...
            "安全骨架、研究结果或\n首个阶段只是进展",
            "Cybion 永远不做长期规划器",
            "requirement graph、coverage\ndatabase、自动阶段拆分或里程碑编排",
        ] {
    ...
    fn checkpoints_are_inclusive_and_subthreads_replay_only_their_own_scope() {
        ...
        assert_eq!(inherited[1]["role"], "developer");
        assert!(
            inherited[1]["content"]
                .as_str()
                .is_some_and(|content| content.contains("Ship the release."))
        );
        assert_eq!(inherited[2]["type"], "function_call");
        assert_eq!(inherited[3]["role"], "user");
        ...
        assert_eq!(
            inherited
                ...
                .filter(|item| item["role"] == "user")
                .collect::<Vec<_>>(),
            vec![&json!({"role": "user", "content": "Verify the release."})]
        );
        assert!(
            !inherited
                .iter()
    ...
    async fn legacy_context_window_settings_do_not_trigger_compaction() {
        async fn responses(
            ...
            let response = json!({
                "output": [{
                    "type": "message",
                    "role": "assistant",
                    "content": [{"type": "output_text", "text": "normal result"}],
                }],
        ...
        open_db(&db)
            .unwrap()
            .execute(
                "INSERT INTO app_meta (key, value) VALUES ('context_window_limit', '4')",
                [],
            )
            .unwrap();
        ...
        let result = run_agent(
            ...
            vec![ChatMessage {
                role: "user".to_owned(),
                content: Value::String("start".to_owned()),
                images: None,
                tool_call_id: None,
                tool_calls: None,
            }],
        ...
        assert_eq!(
            audits
                .items
                .iter()
                .filter(|item| item.request_kind == "normal")
                .count(),
            1
        );
        assert_eq!(
